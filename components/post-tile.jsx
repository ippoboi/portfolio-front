import urlFor from "@app/lib/urfFor";
import Image from "next/image";
import Link from "next/link";

function PostTile({ data }) {
  return (
    <article className="border-primary-black group/card border max-w-lg min-w-lg h-72 flex flex-col justify-end p-5 relative bg-gradient-to-t from-white to-transparent">
      {data?.mainImage && (
        <Image
      ...
      <h2 className="small_spaced_text font-semibold">{data?.title}</h2>
      <p className="text-[14px]">{data?.excerpt}</p>
      <Link
